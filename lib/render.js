...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  .ccards { display: none; margin: 28px 0 0; }
  .ccard { border: 1px solid var(--rule); border-top: 3px solid var(--card-accent, var(--accent));
    border-radius: 4px; background: var(--paper-2); padding: 4px 16px 8px; margin-bottom: 16px; }
  .ccard-head { display: block; font-family: "Fraunces", serif; font-size: 21px; font-weight: 600;
    color: var(--ink); border-bottom: none; padding: 12px 0 8px; }
  .ccard-row { display: flex; justify-content: space-between; gap: 14px;
    padding: 9px 0; border-top: 1px solid var(--rule); }
  .ccard-k { font-family: "IBM Plex Mono", monospace; font-size: 11px; font-weight: 500;
    text-transform: uppercase; letter-spacing: 0.05em; color: var(--muted); flex: 0 0 42%; }
  .ccard-v { font-size: 14px; color: var(--ink); text-align: right; flex: 1; }
...
${videoBlock(s.video)}`
    : s.virtualTour
    ? `    <p>Before reading further, it's worth seeing the place. ${s.shortName} offers an official interactive virtual tour you can explore at your own pace: <a href="${s.virtualTour}">${s.shortName} virtual tour</a>.</p>`
    : `    <p>Before reading further, it's worth exploring the campus on the college's official site.</p>`
}
  </section>
`;
...
<main class="wrap">
  <section id="fuel" style="border-bottom:none;">
    <span class="sec-num">★ / The important stuff</span>
    <h2>Late-night fuel</h2>
    <p>Not academic, but you asked. The closest McDonald's, Domino's, and Chick-fil-A to each campus, with directions. Distances are approximate and chains open and close — tap through to confirm before you make the trip. (Chick-fil-A is closed Sundays.)</p>
    <div class="cards">
    ${order
      .map((slug) => {
        const s = schools[slug];
        const f = s.nearbyFood;
        if (!f) return "";
        const origin = encodeURIComponent(`${s.name}, ${s.town}`);
        const dir = (addr) =>
          `https://www.google.com/maps/dir/?api=1&origin=${origin}&destination=${encodeURIComponent(addr)}`;
        return `<div class="card" style="--card-accent:${s.accent}">
      <p class="name" style="font-size:20px">${s.name}</p>
      <ul class="links" style="margin-top:8px">
        <li><a href="${dir(f.mcdonalds.addr)}" target="_blank" rel="noopener">McDonald's →</a><span class="desc">${f.mcdonalds.addr} · ${f.mcdonalds.miles}</span></li>
        <li><a href="${dir(f.dominos.addr)}" target="_blank" rel="noopener">Domino's →</a><span class="desc">${f.dominos.addr} · ${f.dominos.miles}</span></li>
        ${f.chickfila ? `<li><a href="${dir(f.chickfila.addr)}" target="_blank" rel="noopener">Chick-fil-A →</a><span class="desc">${f.chickfila.addr} · ${f.chickfila.miles}</span></li>` : ""}
      </ul>
    </div>`;
      })
      .join("\n    ")}
...
function renderCompare() {
  ...
  // Mobile layout: one stacked card per school instead of a wide table.
  const compareCards = cols
    .map((s) => {
      const items = keys
        .map(
          (k) =>
            `        <div class="ccard-row"><span class="ccard-k">${k}</span><span class="ccard-v">${s.compare[k] || "—"}</span></div>`
        )
        .join("\n");
      return `      <div class="ccard" style="--card-accent:${s.accent}">
        <a class="ccard-head" href="/${s.slug}">${s.name}</a>
${items}
      </div>`;
    })
    .join("\n");
...
<div class="hero wrap">
  ...
  <h1>Compare the schools</h1>
  <p class="lead">A side-by-side look at the schools on SchoolSkimmer. Three are members of the "Little Three" — Williams, Amherst, and Wesleyan, longtime liberal arts rivals — while Carnegie Mellon is a larger research university with a very different structure. They diverge in ways the table makes concrete.</p>
...
${compareCards}
    ...
    <p>The cleanest way to tell them apart is academic philosophy and structure. <strong>Williams</strong> gives you the most structure among the liberal arts schools — distribution requirements, Oxford-style tutorials, and a January Winter Study term. <strong>Amherst</strong> gives you a pure open curriculum at a small, self-contained college with four neighbors to draw on. <strong>Wesleyan</strong> also has an open curriculum, but at a larger university with a distinctive arts and social-engagement culture. <strong>Carnegie Mellon</strong> is the outlier: a research university where you apply directly into one of seven specialized colleges, trading liberal arts breadth for depth in a chosen field.</p>
    <p>The practical splits are large. The three liberal arts schools enroll around 2,000–3,000 and offer a broad, flexible undergraduate education; CMU enrolls nearly 8,000 and is built around specialized professional colleges. Admissions differ in kind, not just degree: Williams and Amherst admit well under 10% through one process, Wesleyan around 16%, while CMU's ~11% overall rate hides enormous variation by college (its computer science school admits under 5%). And on aid, the Little Three are need-blind and largely no-loan, whereas CMU is need-aware. If you're choosing among these, start with the most basic question — do you want a broad liberal arts education or specialized depth? — because it separates CMU from the other three before any number does.</p>
  </section>
</main>
`;
  html += disclaimer();
