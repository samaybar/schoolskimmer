...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  /* Homepage cards */
  .cards { display: grid; grid-template-columns: repeat(2, 1fr); gap: 18px; margin: 36px 0 0; }
  ...
  .ctable .school-a { border-left: 3px solid #500082; }
  .ctable .school-b { border-left: 3px solid #3f1d70; }
  @media (max-width: 640px) {
  ...
  let rows = keys
    .map((k) => {
      const cells = cols
        .map(
          (s, i) =>
            `<td class="school-${i === 0 ? "a" : "b"}">${s.compare[k] || "—"}</td>`
        )
        .join("");
